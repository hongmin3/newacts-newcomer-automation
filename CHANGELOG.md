...
- REQ-EDU-003: 집중교육 참석 표시로 숫자 `0`과 영문 `o`·`O`를 모두 읽는다. 교육관리
  시트 상단에 `집중교육 → 참석 명단 반영` 메뉴를 추가하고, 실행 뒤 확인·추가·갱신·검토
  필요 건수를 알림창에 표시한다. `3-1` 같은 번호가 날짜로 저장된 줄도 화면에 보이는
  번호를 읽어 올바른 분기 이름을 기록한다. 앞선 실행이 남긴 날짜 문자열 31건은
  원본 번호와 날짜가 맞을 때에만 고치고 다른 4주차 값은 보존한다.
